...
class CircusManager:
    ...
    def __init__(
        self, config_path: Optional[Path] = None, endpoint: str = "tcp://127.0.0.1:5555"
    ) -> None:
        """
    ...
    async def _initialize_watchers(self) -> None:
        ...
        for watcher_name in watchers:
            ...
            if watcher_config:
                self.process_watcher.add_process(
                    watcher_name,
                    {
    ...
    async def _execute_circus_command(
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self._circus_client.call({"command": command, **kwargs})
            )
    ...
    async def start_process(self, process_name: str) -> bool:
        ...
        if response and response.get("status") == "ok":
            ...
            self.process_watcher.update_process_info(
                process_name, status=ProcessStatus.STARTING, started_at=datetime.now()
            )
    ...
    async def stop_process(self, process_name: str) -> bool:
        ...
        if response and response.get("status") == "ok":
            # Update process status
            self.process_watcher.update_process_info(
                process_name, status=ProcessStatus.STOPPING
            )
    ...
    async def restart_process(self, process_name: str) -> bool:
        ...
        if response and response.get("status") == "ok":
            # Update process status
            self.process_watcher.update_process_info(
                process_name, status=ProcessStatus.RESTARTING, started_at=datetime.now()
            )
