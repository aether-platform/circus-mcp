...
class CircusMCPApplication:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not connected:
                logger.warning(
                    "Could not connect to Circus daemon. Some features may not work."
                )
    ...
    async def show_status():
        ...
        try:
            ...
            if "circus_manager" in status_info:
                ...
                click.echo("\nCircus Manager:")
                click.echo(
                    f"  Initialized: {cm_status.get('circus_manager', {}).get('initialized', False)}"
                )
                click.echo(
                    f"  Connected: {cm_status.get('circus_manager', {}).get('connected', False)}"
                )
            ...
            if "mcp_server" in status_info:
                ...
                click.echo(
                    f"  Available Resources: {mcp_status.get('available_resources', 0)}"
                )
