...
class TestSystemIntegration:
    ...
    async def test_full_system_initialization(
        self, mock_circus_client, mock_config_handler, mock_process_watcher
    ):
        """Test full system initialization and integration."""
        with patch(
            "src.circus_manager.manager.CircusClient", return_value=mock_circus_client
        ), patch(
            "src.circus_manager.manager.ConfigHandler", return_value=mock_config_handler
        ), patch(
            "src.circus_manager.manager.ProcessWatcher",
            return_value=mock_process_watcher,
        ):
    ...
    async def test_process_lifecycle_integration(
        self, mock_circus_client, mock_config_handler, mock_process_watcher
    ):
    ...
    async def test_log_processing_integration(
        self, mock_circus_client, mock_config_handler, mock_process_watcher
    ):
...
class TestMCPServerIntegration:
    ...
    async def test_mcp_server_with_circus_manager(
        self, mock_circus_client, mock_config_handler, mock_process_watcher
    ):
    ...
    async def test_mcp_tools_integration(
        ...
        with patch(
            ...
            with patch.object(
                mcp_server.tools, "execute_tool", return_value={"status": "success"}
            ):
                result = await mcp_server.tools.execute_tool(
                    "process_control", {"action": "start", "process_name": "webapp1"}
                )
...
class TestErrorHandlingIntegration:
    ...
    async def test_circus_connection_failure_handling(
        self, mock_config_handler, mock_process_watcher
    ):
        ...
        with patch(
            "src.circus_manager.manager.CircusClient", return_value=mock_client
        ), patch(
            "src.circus_manager.manager.ConfigHandler", return_value=mock_config_handler
        ), patch(
            "src.circus_manager.manager.ProcessWatcher",
...
class TestPerformanceIntegration:
    ...
    async def test_concurrent_process_operations(
        self, mock_circus_client, mock_config_handler, mock_process_watcher
    ):
...
class TestConfigurationIntegration:
    ...
    async def test_configuration_consistency(self, mock_circus_client):
        ...
        with patch(
            ...
            "src.circus_manager.manager.ConfigHandler", return_value=mock_config
        ), patch(
            "src.circus_manager.manager.ProcessWatcher", return_value=mock_watcher
        ):
            circus_manager = CircusManager()
            await circus_manager.initialize()
