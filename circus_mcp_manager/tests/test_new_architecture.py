...
from src.domain.process import (
    ProcessDomainService, LogDomainService, ProcessConfig, LogEntry, LogLevel
)
from src.infrastructure.repositories import InMemoryProcessRepository, InMemoryLogRepository
...
def application_services(mock_circus_client, repositories, domain_services):
    ...
    log_service = LogService(
        log_repo=log_repo,
        domain_service=log_domain_service
    )
...
class TestNewArchitectureIntegration:
    ...
    async def test_full_stack_process_management(self, mcp_controller, mock_circus_client):
        """フルスタックプロセス管理テスト"""
        # プロセス追加
        add_params = {
            "name": "add_process",
        ...
        result = await mcp_controller._execute_tool("add_process", add_params["arguments"])
        assert result["success"] is True
        
        # プロセス開始
        start_params = {
            "name": "start_process",
            "arguments": {"name": "webapp1"}
        }
        
        result = await mcp_controller._execute_tool("start_process", start_params["arguments"])
        assert result["success"] is True
        ...
        result = await mcp_controller._execute_tool("get_process_status", status_params["arguments"])
    ...
    async def test_log_domain_service_classification(self, domain_services):
        ...
        # ログレベル分類テスト
        assert log_domain_service.classify_log_level("ERROR: Something went wrong") == LogLevel.ERROR
        assert log_domain_service.classify_log_level("WARNING: This is a warning") == LogLevel.WARNING
        assert log_domain_service.classify_log_level("INFO: Application started") == LogLevel.INFO
        assert log_domain_service.classify_log_level("DEBUG: Debug information") == LogLevel.DEBUG
        assert log_domain_service.classify_log_level("CRITICAL: System failure") == LogLevel.CRITICAL
    ...
    async def test_error_handling_across_layers(self, mcp_controller, mock_circus_client):
        """レイヤー間のエラーハンドリングテスト"""
        # Circusクライアントエラーをシミュレート
        mock_circus_client.call.side_effect = Exception("Circus connection failed")
        
        # プロセス開始を試行（エラーが適切に処理されることを確認）
        result = await mcp_controller._execute_tool("start_process", {"name": "webapp1"})
        
        # エラーが適切に処理され、レスポンスが返されることを確認
        assert "error" in result or result.get("success") is False
        
        # 存在しないプロセスの操作
        result = await mcp_controller._execute_tool("stop_process", {"name": "nonexistent"})
