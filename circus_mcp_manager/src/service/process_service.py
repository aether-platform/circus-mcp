...
class ProcessService:
    ...
    async def add_process(self, name: str, config: ProcessConfig) -> bool:
        ...
        try:
            ...
            response = await self._execute_circus_command(
                "add", name=name, **circus_config
            )

            if response and response.get("status") == "ok":
                # プロセス情報をリポジトリに保存
                process_info = ProcessInfo(
                    name=name, status=ProcessStatus.STOPPED, config=config
                )
    ...
    async def _execute_circus_command(
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.circus_client.call({"command": command, **kwargs})
            )
