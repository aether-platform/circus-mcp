...
class LogService:
    ...
    def __init__(self, log_repo: LogRepository, domain_service: LogDomainService):
    ...
    async def _handle_alert(self, log_entry: LogEntry) -> None:
        ...
        logger.warning(
            f"ALERT: {log_entry.level.value.upper()} in {log_entry.process_name}: {log_entry.message}"
        )
