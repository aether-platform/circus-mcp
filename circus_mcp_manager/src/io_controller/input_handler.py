...
class InputHandler:
    ...
    async def send_input(self, data: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Queued input for {self.process_name}: {len(sanitized_data)} chars")
            return True
    ...
    async def _process_input_queue(self) -> None:
        ...
        while self._processing:
            try:
                ...
                logger.error(f"Error processing input queue for {self.process_name}: {str(e)}")
                self._stats["processing_errors"] += 1
    ...
    async def _send_to_process(self, input_item: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to send input to process {self.process_name}: {str(e)}")
    ...
    def clear_queue(self) -> int:
        ...
        logger.info(f"Cleared {cleared_count} items from input queue for {self.process_name}")
