...
def validate_process_name(process_name: str) -> bool:
    ...
    if not isinstance(process_name, str):
        raise InvalidProcessNameError(
            str(process_name), "Process name must be a string"
        )
    ...
    if len(process_name) > 64:
        raise InvalidProcessNameError(
            process_name, "Process name too long (max 64 characters)"
        )

    if len(process_name) < 1:
        raise InvalidProcessNameError(
            process_name, "Process name too short (min 1 character)"
        )
    ...
    if process_name.lower() in reserved_names:
        raise InvalidProcessNameError(
            process_name, f"'{process_name}' is a reserved name"
        )
