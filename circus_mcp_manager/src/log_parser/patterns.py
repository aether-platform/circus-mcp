...
class PatternManager:
            ...
            for pattern_info in pattern_list:
                ...
                if regex:
                    try:
                        ...
                        logger.error(f"Invalid custom pattern for level {level}: {str(e)}")
    ...
    def _load_default_patterns(self) -> None:
        ...
        self.patterns = {
            "error": [
                LogPattern(r"ERROR|Exception|Traceback|Fatal|Critical", 1, re.IGNORECASE),
            ...
            "info": [
                LogPattern(r"INFO|Starting|Stopping|Listening|Server", 3, re.IGNORECASE),
                LogPattern(r"\[INFO\]", 3),
            ],
            "debug": [
        ...
        for level, patterns in all_patterns.items():
            for pattern in patterns:
                try:
                    ...
                    errors.append(f"Invalid pattern in level {level}: {pattern.regex_str} - {str(e)}")
