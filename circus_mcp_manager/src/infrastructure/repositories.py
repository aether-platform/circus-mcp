...
from ..domain.process import (
    ProcessInfo, LogEntry, LogLevel,
    ProcessRepository, LogRepository
)
...
class InMemoryLogRepository(LogRepository):
    ...
    def __init__(self, max_logs_per_process: int = 10000):
        self._logs: Dict[str, deque] = defaultdict(lambda: deque(maxlen=max_logs_per_process))
    ...
    def save_log_entry(self, log_entry: LogEntry) -> None:
        ...
        logger.debug(f"Saved log entry for {log_entry.process_name}: {log_entry.level.value}")
