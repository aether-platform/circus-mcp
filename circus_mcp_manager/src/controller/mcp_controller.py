...
class MCPController:
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    await self._send_error_response(None, -32700, f"Parse error: {str(e)}")
                    continue
    ...
    async def _process_message(self, message: Dict[str, Any]) -> None:
        ...
        if not method:
            await self._send_error_response(message_id, -32600, "Invalid request: missing method")
            return
        
        # ハンドラーを検索
        handler = self._handlers.get(method)
        if not handler:
            await self._send_error_response(message_id, -32601, f"Method not found: {method}")
            return
        ...
        try:
            ...
            await self._send_error_response(message_id, -32603, f"Internal error: {str(e)}")
    ...
    async def _send_error_response(self, message_id: Any, code: int, message: str) -> None:
        """エラーレスポンスを送信"""
        response = {
            "jsonrpc": "2.0",
            "id": message_id,
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "num_processes": {"type": "integer", "description": "Number of processes"}
                    },
    ...
    async def _execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if tool_name == "start_process":
                ...
                return {"success": success, "message": f"Process {name} {'started' if success else 'failed to start'}"}
                ...
                return {"success": success, "message": f"Process {name} {'stopped' if success else 'failed to stop'}"}
                ...
                return {"success": success, "message": f"Process {name} {'restarted' if success else 'failed to restart'}"}
                ...
                return {"success": success, "message": f"Process {name} {'added' if success else 'failed to add'}"}
